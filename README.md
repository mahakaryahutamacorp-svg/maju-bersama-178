...
Satu skrip untuk **reset `public` + schema lama `mb178`**, lalu tabel, RLS, dan data awal (8 toko + owner + master admin + tabel **`banners`** untuk slider beranda):
...
5. Jika database sudah jalan tanpa reset penuh dan error *relation banners does not exist*, jalankan sekali [`supabase/add-banners-if-missing.sql`](supabase/add-banners-if-missing.sql).
