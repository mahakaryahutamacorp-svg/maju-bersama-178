...
CREATE TABLE public.orders (
    ...
    SET NULL,
        ...
        -- 'pending', 'pending_payment', 'processing', 'shipped', 'completed', 'cancelled'
        customer_name text,
        customer_phone text,
        notes text,
...
CREATE OR REPLACE FUNCTION public.profile_display_name_from_user_meta(p_meta jsonb, p_email text) RETURNS text LANGUAGE plpgsql IMMUTABLE AS $$ BEGIN RETURN COALESCE(
...
SET search_path = public AS $$ BEGIN -- Populate Members (Mapping for Login & Display)
    BEGIN
...
EXCEPTION
WHEN unique_violation THEN -- Jika username sudah dipakai ID lain, biarkan (tidak crash)
NULL;
END;
RETURN NEW;
END;
...
CREATE OR REPLACE VIEW public.v_etalase_toko WITH (security_invoker = true) AS
SELECT p.*,
...
-- ============================================================
-- Stores: Public Read
CREATE POLICY "stores_read_all" ON public.stores FOR
SELECT USING (true);
-- Products: Public Read, Owner Manage Own Store
CREATE POLICY "products_read_all" ON public.products FOR
SELECT USING (true);
CREATE POLICY "products_owner_manage" ON public.products FOR ALL TO authenticated USING (
...
-- Banners: Public Read
CREATE POLICY "banners_read_all" ON public.banners FOR
SELECT USING (true);
-- Members: Relaxed for testing (Authenticated can see all)
CREATE POLICY "members_read_all" ON public.members FOR
...
CREATE POLICY "memberships_read_access" ON public.store_memberships FOR
...
CREATE POLICY "orders_access" ON public.orders FOR
SELECT TO authenticated USING (
...
CREATE POLICY "order_items_access" ON public.order_items FOR
SELECT TO authenticated USING (
        EXISTS (
            SELECT 1
...
FOR r IN
SELECT *
FROM jsonb_array_elements(p_items) LOOP v_product_id := (r->>'product_id')::uuid;
v_qty := (r->>'qty')::numeric;
SELECT name,
    unit,
    price,
...
WHERE id = v_product_id
    AND store_id = p_store_id FOR
UPDATE;
IF NOT FOUND THEN RAISE EXCEPTION 'Product % not found in store',
v_product_id;
END IF;
IF v_stock < v_qty THEN RAISE EXCEPTION 'Insufficient stock for % (requested %, available %)',
v_name,
v_qty,
v_stock;
END IF;
